...
public class TimedGridPolluter : IGridPolluter
{
	class ToxicSource
	{
	...
	public TimedGridPolluter(WorldGrid grid, float pollutionExpansionTime, float pollutionExpansionTimeVariation)
	{
		m_Grid = grid;
	...
	private void CheckExpandPollution()
	{
		m_Pollution.ForEach(source =>
		{
			...
			source.piece.m_SourceCoordinates.ForEach(coord => m_Grid.GetCoordinateRepresentation(coord)?.UpdateTimer(source.nextExpansionTime - Time.time));
		});
	}
...
//public class TurnBasedGridPolluter : IGridPolluter
//{
//	class ToxicSource
//	{
//		public ToxicPiece piece;
//		public int nextExpansionTurn;
//	}

//	private readonly WorldGrid m_Grid;
//	private readonly List<ToxicSource> m_Pollution = new List<ToxicSource>();

//	private readonly int m_PollutionExpansionTime = 5;
//	private readonly int m_PollutionExpansionTimeVariation = 0;

//	public TurnBasedGridPolluter(WorldGrid grid, int pollutionExpansionTime, int pollutionExpansionTimeVariation)
//	{
//		m_Grid = grid;
//		m_PollutionExpansionTime = pollutionExpansionTime;
//		m_PollutionExpansionTimeVariation = pollutionExpansionTimeVariation;
...
//		pieces.ForEach(piece =>
//		{
//			m_Pollution.Add(new ToxicSource() { piece = piece, nextExpansionTurn = CalculateNextExpansion(m_Grid.m_NumPiecesPlaced) });
...
//		m_Pollution.ForEach(source =>
//		{
//			if (m_Grid.m_NumPiecesPlaced >= source.nextExpansionTurn)
//			{
//				source.piece.Expand();
//				source.piece.RedecorateCords();
//				source.nextExpansionTurn = CalculateNextExpansion(m_Grid.m_NumPiecesPlaced);
//			}
//			source.piece.m_SourceCoordinates.ForEach(coord => m_Grid.GetCoordinateRepresentation(coord)?.UpdateTimer(source.nextExpansionTurn - m_Grid.m_NumPiecesPlaced));
//		});
//	}

//	private int CalculateNextExpansion(int initialTurn) => initialTurn + Random.Range(m_PollutionExpansionTime - m_PollutionExpansionTimeVariation, m_PollutionExpansionTime + m_PollutionExpansionTimeVariation);
//}